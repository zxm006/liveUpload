...
void CRtmpClient::reconnectServer()
{
    if(m_pRtmp)
    {
       
        RTMP_CloseNoDeleteStream(m_pRtmp);
        RTMP_Free(m_pRtmp);
        m_pRtmp = NULL;
    }
    ConnectServer(m_rtmpUrl.c_str(),false);
...
BOOL CRtmpClient::SendH264VideoPacket(unsigned char *pData, unsigned int nSize, unsigned int nWidth, unsigned int nHeight, unsigned int nFrameRate, bool bIsKeyFrame, unsigned int nTimeStamp)
{
    ...
    if(m_isSendSpsInfo)
    {
        ...
        BOOL bre = GetNaluInfo(naluUnit, uDataSize, spsData);
        if(!bre)
            return FALSE;
        ...
        memcpy(metaData.Sps,naluUnit.data,naluUnit.size);
          ...
          bre = GetNaluInfo(naluUnit, uDataSize-naluUnit.size, spsData+naluUnit.size);
        if(!bre)
            return FALSE;
